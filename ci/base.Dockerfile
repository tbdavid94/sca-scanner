...
RUN npm install --no-audit --progress=false --only=production -g @appthreat/cdxgen @microsoft/rush --unsafe-perm

RUN mkdir -p /opt/sl-cli /opt/phpsast && cd /opt/phpsast && composer require --quiet --no-cache --dev vimeo/psalm

RUN composer config --global allow-plugins.phpstan/extension-installer true
RUN composer require --quiet --no-cache --dev phpstan/extension-installer
RUN composer require --quiet --no-cache --dev phpstan/phpstan \
...
#CMD [ "logs analyze -o > reports/logs-output.json" ]
